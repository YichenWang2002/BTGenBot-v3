"""Run pure-navigation benchmark suites."""
...
from src.experiments.summarize import (
    write_summary_csv,
    write_summary_md,
    summarize_rows,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Experiment rows: {len(rows)}")
    return 0
...
def run_suite(suite_path: Path) -> list[dict[str, Any]]:
    ...
    output_dir = Path(suite.get("output_dir", "results/nav_smoke"))
    ...
    for scenario_index, manifest_row in enumerate(manifest_rows[:max_scenarios]):
        ...
        for method in methods:
            run_id = f"{suite.get('name', 'nav')}_{scenario_index}_{method}"
    ...
    write_raw_csv(output_dir / "summary.csv", raw_rows)
    summary_rows = summarize_rows(raw_rows)
    write_summary_csv(output_dir / "summary_by_method.csv", summary_rows)
    write_summary_md(output_dir / "summary.md", summary_rows)
    # User-facing requirement names summary.csv as the aggregate table. Keep a copy there.
...
def build_method_scenario(
    ...
    if method == "single_robot_sequential":
        ...
        centralized_rule = False
    elif method == "centralized_rule_multi_robot":
        robot_ids = sorted(data["robots"])
        robot_starts = {
            robot_id: data["robots"][robot_id]["start"] for robot_id in robot_ids
        }
